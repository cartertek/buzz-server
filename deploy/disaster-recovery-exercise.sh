...
pre=$(/usr/local/bin/buzz-server secrets fingerprint --input /run/buzz-server/credentials/owner-secret)
$controller rotate-owner "$new_owner" "${BUZZ_KMS_KEY_ID:-}"
$controller health
post=$(/usr/local/bin/buzz-server secrets fingerprint --input /run/buzz-server/credentials/owner-secret)
[ "$pre" != "$post" ] || { echo "owner rotation did not change the key fingerprint" >&2; exit 1; }
$controller restore "$backup"
$controller health
restored=$(/usr/local/bin/buzz-server secrets fingerprint --input /run/buzz-server/credentials/owner-secret)
