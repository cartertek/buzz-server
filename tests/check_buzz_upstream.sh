...
grep -Fx 'release_tag=desktop-v0.5.23' <<<"$success"
grep -Fx 'expected_release_commit=b9392d9d78744df365f9276e1ffe8c1baa5ea903' <<<"$success"

annotated=$(GH_BIN="$mock" MOCK_BUZZ_RELEASE_CASE=annotated "$check")
grep -Fx 'expected_release_commit=b9392d9d78744df365f9276e1ffe8c1baa5ea903' <<<"$annotated"

if mismatch=$(GH_BIN="$mock" MOCK_BUZZ_RELEASE_CASE=mismatch "$check" 2>&1); then
  echo 'revision mismatch unexpectedly passed' >&2
  exit 1
fi
grep -F 'pinned=b9392d9d78744df365f9276e1ffe8c1baa5ea903' <<<"$mismatch"
grep -F 'release_tag=desktop-v0.5.23' <<<"$mismatch"
